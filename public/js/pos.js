...
$(document).ready(function () {
  customer_set = false;
  //#king Retrieve selected products from localStorage
  var storedProducts = localStorage.getItem("selectedProducts");

  // Check if storedProducts is not null and not an empty string
  if (storedProducts && storedProducts.trim() !== "") {
    ...
    // Handle the case where storedProducts is null or empty
    selectedProducts = [];
  }

  // #king function to capture products for second screen update
  function captureProductInfo() {
    $("#pos_table tbody tr.product_row").each(function () {
      ...
      var symbolPlacement = $("#__symbol_placement").val();
      var rewardPointEnabled = $("#reward_point_enabled").val();
      var totalDiscount = parseFloat($("#total_discount").text());
      var discountType = $("#discount_type").val();
      var discountAmount = parseFloat($("#discount_amount").val());
      var rpRedeemed = parseFloat($("#rp_redeemed").val());
      var rpRedeemedAmount = parseFloat($("#rp_redeemed_amount").val());
      var orderTax = parseFloat($("#order_tax").text());
      var taxRateId = $("#tax_rate_id").val();
      var taxCalculationAmount = parseFloat($("#tax_calculation_amount").val());
      var shippingChargesAmount = parseFloat(
        $("#shipping_charges_amount").text()
      );
      var prodType = $(".hidden-prod-type").val();
      var finalTotal = parseFloat($("#final_total_input").val());
      var hiddenProdBrand = tr.find("#hidden-prod-brand").val();
      var hiddenProdUnit = tr.find("#hidden-prod-unit").val();
      var hiddenProdSecondUnit = tr.find("#hidden-prod-secondUnit").val();
      var hiddenProdSecondUnitQty = tr.find("#hidden-prod-secondUnitQty").val();
      var quantityInput = tr.find(
        'input[name^="products"][name$="[quantity]"]'
      );
      var quantity = quantityInput.val() || quantityInput.attr("value");
      // #king change this line below if your price doesnt work after qunatity increase
      var unitPriceText = tr.find("td:eq(3)").text().trim();
      console.log("Unit Price after increase quantity: ", unitPriceText);
      console.log(
        "if unit price is blank after qunatity change , go public/js/pos.js line 55 and change td:eq((3) to td:eq(4), hard refresh and try again "
      );
      var hiddenProductVary = tr.find("#hidden-prod-vary").val();
      var hiddenProductType = tr.find("#hidden-prod-type ").val();
      var hiddenProductSku = tr.find("#hidden-prod-sku ").val();
      var hiddenProductPrice = tr.find("#hidden-prod-price ").val();
      var hiddenProductStock = tr.find("#hidden-prod-stock").val();
      var hiddenProductName = tr.find("#hidden-prod-name ").val();
      var hiddenProductQty = tr.find("#hidden-prod-qty ").val();
      var productInfo = {
        name: hiddenProductName,
        quantity: parseFloat(quantity),
        unitPrice: unitPriceText,
        sku: hiddenProductSku,
        code: code,
        symbol: symbol,
        symbolPlacement: symbolPlacement,
        rewardPointEnabled: rewardPointEnabled,
        totalDiscount: totalDiscount,
        discountType: discountType,
        discountAmount: discountAmount,
        rpRedeemed: rpRedeemed,
        rpRedeemedAmount: rpRedeemedAmount,
        orderTax: orderTax,
        taxRateId: taxRateId,
        taxCalculationAmount: taxCalculationAmount,
        shippingChargesAmount: shippingChargesAmount,
        finalTotal: finalTotal,
        variation: hiddenProductVary,
        prodType: prodType,
        // image: prodImage,
        type: hiddenProductType,
        brand: hiddenProdBrand,
        unit: hiddenProdUnit,
        SecondUnit: hiddenProdSecondUnit,
        SecondQty: hiddenProdSecondUnitQty,
      };

      // Push the product into the selectedProducts array
      selectedProducts.push(productInfo);
    });

    // Update the local storage with the selectedProducts array
    localStorage.setItem("selectedProducts", JSON.stringify(selectedProducts));
  }
  // #king second screen capture ends here
  ...
  //#king remove item from table row
  $(document).on("click", "i.pos_remove_row", function () {
    var thisRow = $(this).parents("tr");
    var nameText = thisRow.find("td:eq(0) h4").text().trim();
    nameText = nameText.replace(/[×\t\n\r]/g, "");

    for (var i = selectedProducts.length - 1; i >= 0; i--) {
      var productName = selectedProducts[i].sku;
      if (productName && nameText.includes(productName)) {
        selectedProducts.splice(i, 1);
      }
    }

    // Update the local storage with the updated selectedProducts array
    localStorage.setItem("selectedProducts", JSON.stringify(selectedProducts));

    // Remove all rows with the same product sku from the table
    $('table#pos_table tbody tr:contains("' + nameText + '")').remove();
  ...
  $(
    "select#product_category, select#product_brand, select#select_location_id"
  ...
  // #king refresh the storage when cashier leaves page
    ...
    $.ajax({
      ...
      success: function (result) {
        if (result.success) {
          ...
          //#king capture products entered, thanks to #JCN idea to use the ajax
          var code = $("#__code").val();
          var symbol = $("#__symbol").val();
          var symbolPlacement = $("#__symbol_placement").val();
          var rewardPointEnabled = $("#reward_point_enabled").val();
          ...
          var orderTax = parseFloat($("#order_tax").text());
          var taxRateId = $("#tax_rate_id").val();
          var taxCalculationAmount = parseFloat(
            $("#tax_calculation_amount").val()
          );
          var shippingChargesAmount = parseFloat(
            $("#shipping_charges_amount").text()
          );
          var finalTotal = parseFloat($("#final_total_input").val());
          var hiddenProdBrand = this_row.find("#hidden-prod-brand").val();
          var hiddenProdUnit = this_row.find("#hidden-prod-unit").val();
          var hiddenProdSecondUnit = this_row
            .find("#hidden-prod-secondUnit")
            .val();
          var hiddenProdSecondUnitQty = this_row
            .find("#hidden-prod-secondUnitQty")
            .val();
          var prodType = $(".hidden-prod-type").val();
          var hiddenProductVary = this_row.find("#hidden-prod-vary").val();
          var hiddenProductType = this_row.find("#hidden-prod-type ").val();
          var hiddenProductSku = this_row.find("#hidden-prod-sku ").val();
          var hiddenProductPrice = this_row.find("#hidden-prod-price ").val();
          var hiddenProductStock = this_row.find("#hidden-prod-stock").val();
          var hiddenProductName = this_row.find("#hidden-prod-name ").val();
          var hiddenProductQty = this_row.find("#hidden-prod-qty ").val();
          var productInfo = {
            name: hiddenProductName,
            quantity: hiddenProductQty,
            unitPrice: hiddenProductPrice,
            sku: hiddenProductSku,
            code: code,
            symbol: symbol,
            symbolPlacement: symbolPlacement,
          ...
          selectedProducts.push(productInfo);
          // Update the local storage with the selectedProducts array immediately
          localStorage.setItem(
            "selectedProducts",
            JSON.stringify(selectedProducts)
          );
          // #king product capture ends here
        } else {
          toastr.error(result.msg);
          $("input#search_product").focus().select();
        }
...
function reset_pos_form() {
  ...
  // #king reset the forms when the bottons are clicked
  localStorage.removeItem("selectedProducts");
